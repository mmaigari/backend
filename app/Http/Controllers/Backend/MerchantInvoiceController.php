...
class MerchantInvoiceController extends Controller
{
    ...
    public function InvoiceDetails($merchantId,$invoiceId){
        ...
        return view('backend.merchant.invoice.invoice_details', compact('invoice'));
    }
    ...
    public function InvoiceCSV($merchant_id,$invoice_id){
        ...
        if($this->repo->invoiceGet($merchant_id,$invoice_id) && count($this->repo->invoiceGet($merchant_id,$invoice_id)->parcels) > 0):
             $invoice = $this->repo->invoiceGet($merchant_id,$invoice_id);
             return Excel::download(new InvoiceExport($merchant_id,$invoice_id),'invoice-'.$invoice->merchant->business_name.'-'.$invoice->invoice_date.'.xlsx');
        else:
            Toastr::error(__('parcel.error_msg'),__('message.error'));
            return redirect()->back();
        endif;
