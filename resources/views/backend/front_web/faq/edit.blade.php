        ...
        <div class="  col-sm-12 col-12">
            <div class="card">
                <div class="card-body">
                    ...
                    <form action="{{route('faq.update',$faq->id)}}"  method="POST" enctype="multipart/form-data" id="basicform">
                        @csrf
                        @method('put')
                        <div class="row"> 
                            <div class="form-group col-md-12">
                                <label for="question">{{ __('levels.question') }}</label> <span class="text-danger">*</span>
                                <input id="question" type="text" name="question" data-parsley-trigger="change" placeholder="{{ __('levels.Enter_question') }}" autocomplete="off" class="form-control @error('question') is-invalid @enderror" value="{{old('question',@$faq->question)}}" require>
                                @error('question')
                            ...
                            <div class="form-group    col-md-6">
                                ...
                                <input id="position" type="text" name="position" data-parsley-trigger="change" placeholder="{{ __('placeholder.Enter_Position') }}" autocomplete="off" class="form-control @error('position') is-invalid @enderror" value="{{old('position',$faq->position)}}" >
                                @error('position')
                                    <small class="text-danger mt-2">{{ $message }}</small>
                                @enderror    
                            </div>    
                            <div class="form-group  col-md-6">
                                ...
                                <select name="status" class="form-control @error('status') is-invalid @enderror">
                                    @foreach(trans('status') as $key => $status)
                                        <option value="{{ $key }}" {{ (old('status',$faq->status) == $key) ? 'selected' : '' }}>{{ $status }}</option>
                                    @endforeach    
                                </select>    
                                @error('status')
                                <small class="text-danger mt-2">{{ $message }}</small>
